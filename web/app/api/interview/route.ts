...
export async function POST(req: Request) {
  ...
  if (!KEY) {
    return NextResponse.json({ error: "no_ai_key", message: "AI is not configured. Add ANTHROPIC_API_KEY to enable interview analysis." }, { status: 503 });
  }

  const { transcript, candidateName, role } = await req.json().catch(() => ({}));
...
"""`;
  ...
  try {
    ...
    if (!r.ok) {
      ...
      return NextResponse.json({ error: "ai_failed", message: txt.slice(0, 240) }, { status: 502 });
    }
    const data = await r.json();
    const text: string = (data.content || []).map((c: any) => c.text || "").join("");
    let parsed: any;
    try { const m = text.match(/\{[\s\S]*\}/); parsed = JSON.parse(m ? m[0] : text); }
    catch { return NextResponse.json({ error: "parse", message: "Could not parse the AI response." }, { status: 502 }); }
    ...
    return NextResponse.json({ id: iv.id, ...parsed });
  } catch (err: any) {
